...
def catalog_to_xml(data: Dict[str, Dict]) -> str:
    ...
    def treat_level(level: Dict[str, Any], parent: et._Element):
        ...
        if kind == "resource":
            node.attrib["filepath"] = level["filepath"]
        # Add Metadata
        dict_to_xml(node, {key: value for key, value in level.items() if key not in {"members", "id", "filepath"}})

        for level in level.get("members", []):
            treat_level(level, node)
    ...
    def augment(collection_id: str):
        return {
            ...
            **collections[collection_id],
            "members": [augment(member) for member in link_dicts.get(collection_id, [])]
        }
...
def build(wildcard_path, output: str, workers: int, external_function,
    ...
    #     f.write(et.tostring(catalog, encoding=str))
